...
class Pdf_salidas{
  ...
  public function salidaByType($type)
  {
    $body = "";
    $salidas = $this->salida->selectByType($type);
    $total = $this->salida->countByType($type,true);

    $nombre = $type ? 'Ganchos':'Gomas';
    $tbody = "";
    $i = 1;
    ...
    $body.="
    			<div class='12'>
    			<p><b>Total de salidas:</b> {$total->salidas}</p>
    			<p><b>Total de productos:</b> {$total->cantidad}</p>
    			</div>
          <div class='col-12'>
    ...
    $header = "
              ...
              <div class='col-6 text-center'>
                <h4>INVERPARTS DEL TUY</h4>
                <p>Reporte de salida de {$nombre}</p>
              </div>";
    ...
    $footer = "
            ...
            <td width=\"33%\"><span style=\"font-weight: bold; font-style: italic;\">{DATE d-m-Y}</span></td>
            <td width=\"33%\" align=\"center\" style=\"font-weight: bold; font-style: italic;\">{PAGENO}/{nbpg}</td>
            <td width=\"33%\" style=\"text-align: right; \">{$nombre}</td>
            </tr></table>";
    $css = "
	    body{box-sizing: border-box;font-family: 'Source Sans Pro',sans-serif;}
	    table{border-spacing: 0;border-collapse: collapse;}
    ...
    $mpdf->WriteHTML($css,1);
    $mpdf->WriteHTML($body,2);
    $mpdf->Output("salida_{$nombre}.pdf","D");
  }
}//
